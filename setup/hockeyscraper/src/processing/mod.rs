...
pub fn process_game_infos(db_dir: &std::path::Path) -> GameInfoScraped {
  ...
  gameinfo_file
    .read_to_string(&mut gameinfo_buf)
    .expect("Couldn't read contents of gameinfo.db file");

  println!("Compiled file size: {}", gameinfo_buf.len());

  if gameinfo_buf.len() >= 2 {
    let contents: Vec<InternalGameInfo> =
      serde_json::from_str(&gameinfo_buf).expect("Couldn't de-serialize Game Info db");
    if verify_deserialized_content(&contents) {
      return GameInfoScraped::All(contents);
    } else {
      println!(
        "De-serialized content does not match {} games. Found games: {}",
        GAMES_IN_SEASON,
        contents.len()
      );
      panic!("Exiting");
  ...
  if partials_dir.exists() {
    // means we haven't compiled a full DB yet.
    ...
    if let Some(missing) = check_missing(&flattened) {
      ...
      // we have _all_ gameinfos saved as partials. Save it to compiled file.
    ...
    if game_info_full.exists() {
      ...
      let _bytes_read = f_handle
        .read_to_string(&mut buf)
        .expect(format!("Couldn't read contents of {}", game_info_full.display()).as_ref());
      let data: Vec<InternalGameInfo> =
        serde_json::from_str(&buf).expect("Couldn't de-serialize data from Game Info DB file");
